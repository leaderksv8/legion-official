export function renderActivities(data, lang) {
    ...
    c.innerHTML = data.map(i => `
        <div class="b2-item-card">
            <i class="fa-solid fa-${i.icon} b2-icon-box"></i>
            <h4>${i.title[lang]}</h4>
            <p>${i.desc[lang]}</p>
        </div>
    `).join('');
...
export function renderPartners(data) {
    ...
    const html = (items) => items.map(p => `
        <div class="b4-kinetic-item">
            <a href="${p.link}" target="_blank"><img src="${p.img}" alt="P" onerror="this.src='https://placehold.co/400x200?text=Logo'"></a>
        </div>
    `).join('').repeat(4);
...
export function renderTeam(data, lang) {
    ...
    c.innerHTML = data.map(m => `
        <div class="b5-specialist-card">
            <div class="b5-photo-container"><img src="${m.img}" alt="T" onerror="this.src='https://placehold.co/200x200?text=Photo'"></div>
...
export function renderStories(data, lang) {
    ...
    if (!c || !data) return;
    c.innerHTML = data.map(s => `<div class="b6-card"><div class="b6-quote-mark">“</div><p class="b6-card-text">${s.text[lang]}</p><h4>${s.name}</h4></div>`).join('');
}

export function renderNews(data, lang) {
    const c = document.getElementById('news-container');
    if (!c || !data) return;
    c.innerHTML = data.map(n => `<a href="${n.link}" class="b7-news-item" target="_blank"><h4>${n.title[lang]}</h4><span class="b7-date">${n.date}</span></a>`).join('');
}
