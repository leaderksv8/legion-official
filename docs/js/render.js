...
export function renderNews(data, lang) {
    ...
    container.innerHTML = data.map((n, index) => `
        <a href="${n.link}" class="b7-news-item" style="transition-delay: ${index * 0.1s}">
...
export function renderAlbums(data, lang) {
    ...
    container.innerHTML = data.slice(0, 3).map((a, index) => `
        <div class="b7-album-stack" onclick="window.openGallery('${a.id}')" style="transition-delay: ${0.3 + index * 0.1s}">
